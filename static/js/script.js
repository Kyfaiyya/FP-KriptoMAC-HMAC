...
$(document).ready(function () {
    ...
    function displayVerifyResults(response) {
      ...
      $("#verifyResults").html(resultsHtml);
    }

    function displayFileResults(response) {
      let fileSizeKB = (response.file_size / 1024).toFixed(2);
      // Untuk file results, kita bisa menggunakan layout yang mirip dengan generate MAC versi non-carousel
      // atau versi carousel jika Anda mau. Di sini saya gunakan non-carousel untuk variasi.
      let resultsHtml = `
        <div class="card result-card shadow-sm">
          <div class="card-header bg-light border-bottom">
            <h5 class="mb-0"><i class="fas fa-file-shield me-2"></i>File Integrity MACs</h5>
          </div>
          <div class="card-body p-4">
            <p><strong>File:</strong> ${response.filename} (${fileSizeKB} KB)</p>
            <hr>`; // Pemisah setelah info file

      const algorithms = Object.entries(response.results);
      if (algorithms.length === 0) {
        resultsHtml += `<p class="text-muted text-center">No MACs generated for the file.</p>`;
      } else {
        algorithms.forEach(([algorithm, methods], index) => {
          resultsHtml += `
            <div class="algorithm-section ${index < algorithms.length - 1 ? 'mb-4 pb-3' : 'mb-0'}">
              <h6 class="algorithm-title mb-3">
                <span class="badge bg-info algorithm-badge px-3 py-2">${algorithm.toUpperCase()}</span>
              </h6>
          ...
          if (index < algorithms.length - 1) {
            resultsHtml += `<hr class="my-4">`; // Gunakan my-4 atau app-hr jika didefinisikan di CSS
          }
        });
      }
      resultsHtml += `</div></div>`;
    ...
    function displayComparisonResults(response) {
      ...
      $("#comparisonResults").html(resultsHtml);
    }
